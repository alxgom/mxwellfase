...
def swipe(m,k,mu,Dphi0,d,g,D0,wf):
    ...
    print 'total number of steps: %s' %((wfmax-wfmin)/h)
    if stat==0:    
...
if not os.path.exists(os.path.dirname(filename)):
    try:
        ...
        if exc.errno != errno.EEXIST:
            raise
binwrite=open('Results/%d_%d_%d-%d.%d.%d-w_max.in' %localtime()[0:6] ,'wb')
...
ax1.plot(np.array(w_peks)*wscale,strobo_peak,',b')
...
if save==True: 
    fname='%d_%d_%d-%d.%d.%d-strobo_vs_w.png' % localtime()[0:6]
